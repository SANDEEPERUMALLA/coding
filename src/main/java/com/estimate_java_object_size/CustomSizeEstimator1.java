...
public class CustomSizeEstimator1 {

    private CustomSizeEstimator1() {
    }
    ...
    private static int sizeOf(Object obj, IdentityHashMap<Object, Boolean> visited) {
        ...
        size += unsafeSizer.getObjectSize(obj);
        if (clazz.isArray()) {
            int length = Array.getLength(obj);
            for (int i = 0; i < length; ++i) {
                size += sizeOf(obj, visited);
            }
        } else {
            while (clazz != Object.class) {
                for (Field field : getFieldsForClass(clazz)) {
                    ...
                    if (fieldValue == null)
                        continue;
                    Pair<Integer, Boolean> sizeTypePair = checkKnownTypesAndGetSize(fieldValue, field.getType(), visited);
