...
public class CustomSizeEstimator {

    private CustomSizeEstimator() {
    }
    ...
    public static int sizeOf(Object obj) {
        ...
        return sizeOf(obj, visited);
    }

    private static int sizeOf(Object obj, IdentityHashMap<Object, Boolean> visited) {
        ...
        if (isPrimitive(clazz)) {
            size += getSizeOfPrimitive(clazz);
            return size;
        }

        Pair<Integer, Boolean> sizeTypePair = checkKnownTypesAndGetSize(obj, clazz, visited);

        if (sizeTypePair.getRight()) {
            return sizeTypePair.getLeft() + SIZE_OF_REF;
        }
        ...
        while (clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                if (isPrimitive(field.getType())) {
                    size += getSizeOfPrimitive(field.getType());
                    continue;
                }
                ...
                sizeTypePair = checkKnownTypesAndGetSize(fieldValue, field.getType(), visited);
                if (sizeTypePair.getRight()) {
                    size += sizeTypePair.getLeft() + SIZE_OF_REF;
                    continue;
                }
                size += sizeOf(fieldValue, visited);
            }
    ...
    private static Pair<Integer, Boolean> checkKnownTypesAndGetSize(Object obj, Class<?> clazz, IdentityHashMap<Object, Boolean> visited) {
        ...
        if (obj instanceof String) {
            ...
            for (Object e : (Collection<?>) obj) {
                size += sizeOf(e, visited);
            }
        } else if (obj instanceof Map<?, ?>) {
            knownType = true;
            for (Map.Entry<?, ?> e : ((Map<?, ?>) obj).entrySet()) {
                size += sizeOf(e.getKey(), visited);
                size += sizeOf(e.getValue(), visited);
            }
        }
